---
doc_type: decision_index
status: current
canonical: true
owner: openagentx
updated_at: 2026-09-14
---

# OpenAgentX 架构决策记录索引

## 架构决策记录

- [ADR-001: OpenAgentX 组织控制面、Resident Worker 与移动指挥台决策](ADR-001-resident-agent-worker-runtime-observability.md)
- [ADR-002: Runtime 网络配置、应用与诊断](ADR-002-agent-cli-proxy-environment.md)
- [ADR-003: 指挥台任务详情、运行观察与安全内容呈现](ADR-003-command-center-task-observation-and-content.md)
- [ADR-004: 指挥台网络配置体验极简收敛与路由器模型](ADR-004-streamlined-command-center-network-ux.md)
- [ADR-005: Fleet、tmux Console Attach 与 Worker 开发者体验](ADR-005-interactive-worker-console-and-developer-experience.md)
- [ADR-006: Task intent 与可验证终态语义](ADR-006-task-intent-and-verifiable-terminal-semantics.md)
- [ADR-007: Worker 网络绑定的代际连续性](ADR-007-worker-network-binding-generation-continuity.md)
